...
public class InMemoryFileSystemTests
{
    ...
    [Theory]
    [ClassData(typeof(SuccessFileSystemData))]
    public void SuccessTest(string consoleString)
    {
        var mock = new Mock<ICommandEventLiteners>();
        mock.Setup(c => c.Reaction());

        ICommand? command = _parser.Parse(consoleString);
        Assert.NotNull(command);
        command.Events.Subscribe(CommandStatus.Success, mock.Object);

        mock.Verify(c => c.Reaction(), Times.AtMostOnce);
    }

    [Theory]
    [ClassData(typeof(ErrorFileSystemData))]
    public void ErrorTest(string consoleString)
    {
        var mock = new Mock<ICommandEventLiteners>();
        mock.Setup(c => c.Reaction());
        ...
        ICommand? command = _parser.Parse(consoleString);
        Assert.NotNull(command);
        command.Events.Subscribe(CommandStatus.Error, mock.Object);
